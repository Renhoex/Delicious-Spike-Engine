...
var groundNormal = -gravityDirection; # ground angle detection (used for slopes)
...
func _process(delta):
	...
	if (!locked):
		...
		# check that jump key is pressed and air jumps is above 0 or if the player is on the floor
		if (Input.is_action_just_pressed("gm_jump") && (airJump > 0 || ground || (jumpArea.size() > 0 && jumpArea[0].get("type")  == null))):
			...
			if (ground || coyoteTime > 0 || (jumpArea.size() > 0 && jumpArea[0].get("type") == null)):
				...
				velocity.y = -PlayerPhysics.jumpStrngth*gravMultiplier;
				# Play jump sound
			...
			# perform double jump if we're not on a vine (this prevents using the second jump and wall jump at the same time)
				...
				# Set vertical velocity to the double jump strength
				velocity.y = -PlayerPhysics.dJumpStrngth*gravMultiplier;
		...
		if (Input.is_action_just_released("gm_jump") && (velocity.y < 0) || is_on_ceiling()):
			velocity.y *= 0.35;
		...
		if (Input.is_action_pressed("gm_right")):
			...
			velocity.x = PlayerPhysics.walkSpeed*moveDir*walkMultiplier;
		...
		if (vine != null && !ground):
			...
			if ((Input.is_action_pressed("gm_left") && sprite.scale.x < 0) || (Input.is_action_pressed("gm_right") && sprite.scale.x > 0) || Input.is_action_just_pressed("gm_jump")):
				...
				if (Input.is_action_pressed("gm_jump")):
					# set the players velocity to bounce off the wall using the plaayer jump strength
					velocity = Vector2(PlayerPhysics.walkSpeed*sign(sprite.scale.x)*moveDir,-PlayerPhysics.jumpStrngth);
	...
	if (!overWriteAnimations):
		...
		if ground || coyoteTime > 0:
			# if velocity is 0 then play the idle animation
			if (velocity.x == 0):
				animator.play("Idle");
		...
		# else play air animations
			...
			# else if velocity is goign up then play jump
			elif (velocity.y < 0):
				animator.play("Jump");
...
# physics (this is where velocity actually comes in)
func _physics_process(delta):
	...
	if (!lockedPhysics):
		...
		if ground:
			...
			airJump = PlayerPhysics.totalJumps;
			# reset velocity if it's above 0 (if it's below we want to ignore so the player can jump)
			if (velocity.y > 0):
				velocity.y = min(velocity.y,0);
			# reset coyote time
			coyoteTime = PlayerPhysics.COYOTETIME;
		#elif ((velocity*gravityDirection).length() < (PlayerPhysics.maxFall*gravityDirection.length()) || (velocity*gravityDirection).normalized() != gravityDirection.normalized()):
		else:
			# if not on the ground
			# if on a vine
			if (vine != null):
				# overtake the velocity to prevent jittering
				velocity = Vector2(0,PlayerPhysics.vineFall)*gravMultiplier;
			# else just apply vertical velocity if the falling speed is less then max fall
			elif velocity.y < PlayerPhysics.maxFall*gravMultiplier:
				velocity.y += PlayerPhysics.gravity*delta*gravMultiplier;
			#velocity += gravityDirection*PlayerPhysics.gravity*delta;
		
		# snap is used to tell how far down to check for the floor to snap to, this helps make slopes
		var snap = gravityDirection*4*gravMultiplier;
		# set snap to 0 if we're not moving, this stops the player slipping down slopes
		if ground && velocity.x == 0 && get_floor_normal() != groundNormal:
			snap = Vector2.ZERO;
		...
		# set ground normal to whatever gravity direction is
...
func _on_WaterMask_area_shape_entered(_area_id, area, _area_shape, _self_shape):
	...
	if (area.get_parent().get("gravMultiplier")):
		if (gravMultiplier != area.get_parent().gravMultiplier):
			velocity *= area.get_parent().gravMultiplier;
