extends Label
@export_enum ("SFX", "Music", "VSync", "Display Type", "key Maps", "Auto Key", "Auto Pad", "Back")var type = 0
...
func _process(delta):
	...
	match(type):
		...
		2: # VSync
			text = "VSync: "+str((DisplayServer.window_get_vsync_mode() != DisplayServer.VSYNC_DISABLED))
		3: # Options
