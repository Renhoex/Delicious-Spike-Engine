	...
	if (InputMap.has_action(action) && !locked && actionWait == ""):
		if type == 4: # key
			...
			for i in InputMap.action_get_events(action):
				...
				# check if last
				if (i != InputMap.action_get_events(action)[InputMap.action_get_events(action).size()-1]):
					text += ", "
	
	# action wait release, used for auto map checking
	if (actionWait != ""):
		# compare action wait and new bind
		if (actionWait != event_to_text(InputMap.action_get_events(lastAction)[options.auto-1])):
			actionWait = ""
			lastAction = ""
			activate()
...
func _unhandled_key_input(event):
	...
	if (locked && (options.auto == 0 || options.auto == 1)):
		var rememberGamepad = InputMap.action_get_events(action)[1]
		...
		for i in InputMap.get_actions():
			...
			if (str(i).begins_with(prefix) && event.as_text() == InputMap.action_get_events(i)[0].as_text() && i != action):
				var rememberSwapGamepad = InputMap.action_get_events(i)[1]
				# erase end point (i is the action)
				InputMap.action_erase_events(i)
				# set end point
				InputMap.action_add_event(i, InputMap.action_get_events(action)[0])
				InputMap.action_add_event(i, rememberSwapGamepad)
		
		# check if already current action
		if (event.as_text() != InputMap.action_get_events(action)[0].as_text()):
			# erase inputs
...
func _input(event):
	if event is InputEventJoypadButton || event is InputEventJoypadMotion:
		...
		if (locked && (options.auto == 0 || options.auto == 2)):
			var rememberKey = InputMap.action_get_events(action)[0]
			# check through actions for any duplicates then remap that action to this one if a duplicate exists
			for i in InputMap.get_actions():
				# check if there's more then 1 input
				if InputMap.action_get_events(i).size() > 1:
					# check for prefixes and inputs match
					if (str(i).begins_with(prefix) && event_to_text(event) == event_to_text(InputMap.action_get_events(i)[1]) && i != action):
						var rememberSwapKey = InputMap.action_get_events(i)[0]
						# erase end point (i is the action)
						InputMap.action_erase_events(i)
						# set end point
						InputMap.action_add_event(i, rememberSwapKey)
						InputMap.action_add_event(i, InputMap.action_get_events(action)[1])
			
			# check if already current action
			if (event_to_text(event) != event_to_text(InputMap.action_get_events(action)[1])):
				# erase inputs
				InputMap.action_erase_events(action)
				# set old key input
